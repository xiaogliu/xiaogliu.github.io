...
**进行`hexo`命令操作前，确保本地已安装hexo命令行工具**
...
> 实际不需要重新npm install hexo、npm install、npm install hexo-deployer-git，因为执行npm install，hexo,hexo-deployer-git就已经安装，因为package.json文件里都写了相应依赖了，如下
...
问题描述： 之前使用Ok，忽然就`command not found: hexo`，可能的原因是改变了 node 版本。   
